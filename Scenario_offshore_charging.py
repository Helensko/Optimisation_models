...
OUT_DIR = 'results/offshore_charging_scenario/'
...
df_routes_raw  = pd.read_excel('routes_offshore_charging.xlsx')
df_vessels_raw = pd.read_excel('vessels_3_offshore_charging.xlsx')
...
def solve_shipping_model(df_routes, df_vessels, params=None, force_carrier=None, verbose=False):
    ...
    N_v_LO           = dict(zip(df_vessels['vessel_id'], df_vessels['voyages_per_year_lo']))  # original (pre-doubled) voyages for LO cost
    ...
    if model.status == GRB.OPTIMAL:
        ...
        cost_lo_out    = sum(N_v_LO[v] * C_LO[carrier_by_route[route_of[v]], v].X for v in vessels)
        ...
        for r in routes:
            ...
            cs_r  = sum(
                ...
                for v in V_r)
            clo_r = sum(N_v_LO[v] * C_LO[c, v].X for v in V_r)
...
# Plot all curves first so ylim is data-driven
for _i, _rid in enumerate(_route_ids):
    ...
    ax_s3.plot(_length_nm, _dk_arr, color=_palette[_i], lw=2, label=_label)
...
for _i, _rid in enumerate(_route_ids):
    ...
    if _cx and _length_nm[0] <= _cx <= _length_nm[-1]:
        ...
        ax_s3.text(_cx, ylo + (0 - ylo) * 0.03, f'{_cx:.0f} nm',
                   ha='center', va='bottom', fontsize=7, color=_palette[_i])
...
ax_s3.set_ylabel('Cost difference: $C_{H_2} - C_{bat}$ (kUSD/year)')
